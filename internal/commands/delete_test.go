...
// uuidP1 is the uuid of mockDeleteServer's single known project.
const uuidP1 = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa"

// mockDeleteServer serves GET /v1/project/{uuid}, GET /v1/project/lookup,
// and DELETE /v1/project/{uuid} for a single known project (uuid uuidP1),
// recording every deleted uuid into capture.
func mockDeleteServer(t *testing.T, capture *[]string) *httptest.Server {
	t.Helper()
	project := proj{"uuid": uuidP1, "name": "Product A", "version": "1.0.0", "active": true}
...
func TestProjectDelete_ByUUID_Confirmed(t *testing.T) {
	...
	opts := &projectDeleteOptions{byUUID: uuidP1}
	if err := runProjectDelete(context.Background(), newTestClient(t, srv.URL), opts, strings.NewReader("y\n"), &out); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(deleted) != 1 || deleted[0] != uuidP1 {
		t.Errorf("expected %s to be deleted, got %v", uuidP1, deleted)
	}
	if !strings.Contains(out.String(), "Deleted project Product A 1.0.0 (uuid: "+uuidP1+")") {
...
func TestProjectDelete_ByNameVersion_Yes(t *testing.T) {
	...
	if err := runProjectDelete(context.Background(), newTestClient(t, srv.URL), opts, strings.NewReader(""), &out); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(deleted) != 1 || deleted[0] != uuidP1 {
		t.Errorf("expected %s to be deleted, got %v", uuidP1, deleted)
...
func TestProjectDelete_AbortOnNo(t *testing.T) {
	...
	var out strings.Builder
	opts := &projectDeleteOptions{byUUID: uuidP1}
	if err := runProjectDelete(context.Background(), newTestClient(t, srv.URL), opts, strings.NewReader("n\n"), &out); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(deleted) != 0 {
...
func TestProjectDelete_MissingIdentification(t *testing.T) {
	...
	err := runProjectDelete(context.Background(), newTestClient(t, srv.URL), opts, strings.NewReader(""), &out)
...
func TestProjectDelete_ByUUIDNotFound(t *testing.T) {
	...
	var out strings.Builder
	opts := &projectDeleteOptions{byUUID: uuidUnknown, yes: true}
	err := runProjectDelete(context.Background(), newTestClient(t, srv.URL), opts, strings.NewReader(""), &out)
	if err == nil {
